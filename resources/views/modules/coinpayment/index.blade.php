            ...
            <div class="card">
              ...
              <table class="table">
                <thead>
                  <tr>
                    <th>Description</th>
                  <p id="user-coin">{{Auth::user()->accountType->name}}</p>
    ...
    methods:{
      getMethods(){
        ...
        axios.get('{{ route('coinpayment.ajax.rate.usd', $data['amountTotal']) }}')
          .then(function(json){
            var coin_name = $('#user-coin').text();
            var coin = json.data.coins_accept.filter(function(coin){
              return coin.name == coin_name;
            })
            console.log(coin, coin_name)
            self.paymentMethod = coin[0].iso;
            self.total_amount_coin = coin[0].rate;
      ...
      selectMethod(item){
        ...
        if(item.name == coin_name){
          ...
          swal('Danger!', 'Please kindly use the coin you used to setup your account! Contact the admin if you wish to change your default coin');
        }
      },
      filterCoin: function() {
        var self = this;
