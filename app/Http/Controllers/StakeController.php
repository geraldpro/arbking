...
class StakeController extends Controller{
  ...
  public function resolveStake($lay, $back, $total)
  {
    ...
    // dd($lay_percentage, $back_percentage);
    $total_stake = round(($total * 100) / $lay_percentage, 2);
    $back_amount = round($total_stake * ($back_percentage / 100), 2);
    $back_win = round($back_amount * $back) - $back_amount; 
    $back_lose = $back_amount;
    $lay_win = $total;
    $lay_lose = round($total * $lay, 2) - $total;
    $i_total = $lay_win - $back_lose;
    $b_total = $back_win - $lay_lose;
